...
def fuse_predictions(bert_result: dict,
    ...
    s_fake = svm_result.get("fake_prob",  50.0)
    s_real = svm_result.get("real_prob",  50.0)
    ...
    fused_fake = round(b_fake * BERT_WEIGHT +
                       s_fake * SVM_WEIGHT, 2)
    fused_real = round(b_real * BERT_WEIGHT +
                       s_real * SVM_WEIGHT, 2)
...
def run_fusion(text: str,
    ...
    try:
        ...
        bert_result = {"fake_prob": 50, "real_prob": 50,
    ...
    if bert_ok and svm_ok:
        ...
        model_status = "SVM Only (BERT failed)"
    else:
